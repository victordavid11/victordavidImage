...
const Lifestyle = () => {
  ...
  const openModal = (index) => {
    ...
    setSelectedImage(LifeStyle[index].img);
  };
  ...
  const prevImage = () => {
    const newIndex = (currentIndex - 1 + LifeStyle.length) % LifeStyle.length;
    setCurrentIndex(newIndex);
    setSelectedImage(LifeStyle[newIndex].img);
  };

  // Navigate to next image
  const nextImage = () => {
    const newIndex = (currentIndex + 1) % LifeStyle.length;
    setCurrentIndex(newIndex);
    setSelectedImage(LifeStyle[newIndex].img);
  };
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black/90 flex justify-center items-center z-50">
          ...
          <div className="absolute bottom-4 left-4 text-white bg-black/50 px-3 py-1 rounded-lg">
            {currentIndex + 1} / {LifeStyle.length}
          </div>
        </div>
      )}
    </div>
